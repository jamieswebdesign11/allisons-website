...
<div class="page-content">
    ...
    <div id="feature">
        <div class="feature-inner flex-display">
            <?php while(have_rows('feature')): the_row(); 
            $image = get_sub_field('image'); $title = get_sub_field('title'); $link = get_sub_field('link');
            ?>
            <div class="feature-box flex-col" <?php echo $image ? 'style="background-image:url('. $image['url'] .')"' : ''; ?>>
                <?php echo $link ? '<a href="'. $link['url'] .'" title="'. $link['title'] .'" alt="'. $link['title'] .'">' : ''; ?>
                <?php echo $title ? '<span class="feature-title">'. $title .'</span>' : ''; ?>
                <?php echo $link ? '</a>' : ''; ?>
            </div>
            <?php endwhile; ?>
        </div>
